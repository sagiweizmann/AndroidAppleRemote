...
class RemoteAccessibilityService : AccessibilityService() {
    ...
    private fun navigateOrTap(command: RemoteCommand): Boolean {
        ...
        val fallback = if (command == RemoteCommand.OK) clickFocused() else moveFocus(when (command) {
            RemoteCommand.UP -> View.FOCUS_UP; RemoteCommand.DOWN -> View.FOCUS_DOWN
            RemoteCommand.LEFT -> View.FOCUS_LEFT; RemoteCommand.RIGHT -> View.FOCUS_RIGHT
    ...
    private fun performNavigationGesture(command: RemoteCommand): Boolean {
        ...
        if (command == RemoteCommand.OK) {
            ...
            return dispatchGesture(gesture, object : GestureResultCallback() {
                override fun onCompleted(g: GestureDescription?) { setNavDiagnostic("Command: OK\nMethod: center tap gesture\nCallback: COMPLETED"); CompanionPythonBridge.onStatus("NAV • OK • gesture-completed") }
                override fun onCancelled(g: GestureDescription?) { setNavDiagnostic("Command: OK\nMethod: center tap gesture\nCallback: CANCELLED"); CompanionPythonBridge.onStatus("NAV • OK • gesture-cancelled") }
            }, null)
        }
        ...
        return dispatchGesture(gesture, object : GestureResultCallback() {
            override fun onCompleted(g: GestureDescription?) { setNavDiagnostic("Command: $command\nMethod: swipe gesture\nCallback: COMPLETED"); CompanionPythonBridge.onStatus("NAV • $command • gesture-completed") }
            override fun onCancelled(g: GestureDescription?) { setNavDiagnostic("Command: $command\nMethod: swipe gesture\nCallback: CANCELLED"); CompanionPythonBridge.onStatus("NAV • $command • gesture-cancelled") }
        }, null)
    }

    private fun moveFocus(direction: Int): Boolean {
        val root = rootInActiveWindow ?: return false
        val current = root.findFocus(AccessibilityNodeInfo.FOCUS_INPUT) ?: root.findFocus(AccessibilityNodeInfo.FOCUS_ACCESSIBILITY) ?: firstFocusable(root) ?: return false
        val next = try { current.focusSearch(direction) } catch (_: Throwable) { null } ?: spatialNeighbor(root, current, direction) ?: return false
        return next.performAction(AccessibilityNodeInfo.ACTION_FOCUS) || next.performAction(AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS)
    }

    private fun spatialNeighbor(root: AccessibilityNodeInfo, current: AccessibilityNodeInfo, direction: Int): AccessibilityNodeInfo? {
        val nodes = mutableListOf<AccessibilityNodeInfo>(); collectFocusable(root, nodes)
        val cr = Rect().also(current::getBoundsInScreen); val cx = cr.centerX(); val cy = cr.centerY()
        return nodes.asSequence().filter { it != current }.mapNotNull { n ->
            val r = Rect().also(n::getBoundsInScreen); val dx = r.centerX() - cx; val dy = r.centerY() - cy
            val valid = when (direction) { View.FOCUS_UP -> dy < 0; View.FOCUS_DOWN -> dy > 0; View.FOCUS_LEFT -> dx < 0; View.FOCUS_RIGHT -> dx > 0; else -> false }
            if (!valid) null else {
    ...
    private fun firstFocusable(node: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        ...
        for (i in 0 until node.childCount) node.getChild(i)?.let { firstFocusable(it)?.let { found -> return found } }
        return null
    }
    private fun clickFocused(): Boolean {
        val root = rootInActiveWindow ?: return false
        var node = root.findFocus(AccessibilityNodeInfo.FOCUS_INPUT) ?: root.findFocus(AccessibilityNodeInfo.FOCUS_ACCESSIBILITY) ?: firstFocusable(root) ?: return false
        while (true) { if (node.isClickable && node.performAction(AccessibilityNodeInfo.ACTION_CLICK)) return true; node = node.parent ?: break }
        return false
    }
