...
class AndroidPairingWindow:
    def __init__(self,path,server_identifier,server_generation):
        self.path=path; self.state_dir=path.parent; self.server_identifier=server_identifier; self.server_generation=server_generation
        self._record=_WindowRecord(PIN,time.time()+86400,secrets.token_hex(16),server_identifier,server_generation); self._save()
...
class AndroidCompanionService(FakeCompanionService):
    ...
    def __init__(self,state,*,identifier,private_key,server_generation,pairing_window,paired_clients):
        super().__init__(state,device_name=DEVICE_NAME,unique_id=identifier,private_key=private_key,server_identity_generation=server_generation,paired_clients=paired_clients,require_paired=True,pairing_window=pairing_window)
        type(self)._seq += 1; self.cid=type(self)._seq; self._android_touch_start=None; self._rx=0; self._tx=0
    ...
    def connection_lost(self,exc):
        trace(f"TCP #{self.cid} DISCONNECT • rx={self._rx} tx={self._tx} • {type(exc).__name__+': '+str(exc) if exc else 'peer closed'}")
    ...
    def send_to_client(self,frame_type,data):
        self._tx+=1; name=getattr(frame_type,'name',str(frame_type)); trace(f"TCP #{self.cid} TX#{self._tx} • {name} • encrypted={'YES' if self.chacha else 'NO'}")
    ...
    def enable_encryption(self,output_key,input_key):
        ...
        result=super().enable_encryption(output_key,input_key); trace(f"TCP #{self.cid} • ENCRYPTION ACTIVE"); return result
    ...
    def handle_command_frame(self,frame_type,data):
        trace(f"OPACK COMMAND • {getattr(frame_type,'name',frame_type)}")
...
def run_server(identifier):
    ...
    state_dir=Path(str(Bridge.getStateDir()))/"companion-v3";state_dir.mkdir(parents=True,exist_ok=True)
    private_key=hashlib.sha256(("AndroidAppleRemote:Xiaomi:v3:"+identifier).encode()).digest(); server_generation=hashlib.sha256(("generation:v3:"+identifier).encode()).hexdigest()[:32]
    window=AndroidPairingWindow(state_dir/"pairing-window.json",identifier,server_generation); clients=AndroidPairedClients(state_dir/"paired-clients.json");state=FakeCompanionState()
    trace(f"STATE • PIN {PIN} • paired clients {clients.count()}")
